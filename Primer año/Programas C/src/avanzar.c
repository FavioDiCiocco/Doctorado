...
double Dinamica_interes(ps_Red ps_variable, ps_Param ps_parametro){
	...
	d_resultado = -ps_variable->pd_Opiniones[ps_variable->i_agente*i_C+ps_variable->i_topico+2] * (1+ps_variable->pd_Saturacion[ps_variable->i_agente*i_C+ps_variable->i_topico+2]) + ps_parametro->d_kappa*(d_sumatoria/i_grado);
	return d_resultado;
}
