...
int main(int argc, char *argv[]){
	...
	ps_datos->d_lambda = strtof(argv[2],NULL); // Este parámetro mide la memoria de los agentes respecto de sus intereses previos. Mientras más grande, menos memoria.
	int i_iteracion = strtol(argv[3],NULL,10); // Número de instancia de la simulación.
	...
	ps_datos->d_alfa = 7; // Controversialidad de los tópicos
	ps_datos->d_chi = 2; // Este es el umbral que determina si el interés del vecino puede generarme más interés.
	...
	if(ps_datos->i_N > 5) i_testigos = 5;
	...
	char s_archivo1[355];
	sprintf(s_archivo1,"../Programas Python/Variacion_olvido/Datos/Opiniones_N=%d_Lambda=%.3f_Iter=%d.file"
		,ps_datos->i_N,ps_datos->d_lambda,i_iteracion);
	...
	char s_archivo2[355];
	sprintf(s_archivo2,"../Programas Python/Variacion_olvido/Datos/Testigos_N=%d_Lambda=%.3f_Iter=%d.file"
		,ps_datos->i_N,ps_datos->d_lambda,i_iteracion);
	FILE *pa_archivo2=fopen(s_archivo2,"w"); // Con esto abro mi archivo y dirijo el puntero a él.
	
	// Este archivo es el que guarda las opiniones de todos los agentes del sistema.
	char s_archivo3[355];
	sprintf(s_archivo3,"../Programas Python/Variacion_olvido/Datos/Saturacion_N=%d_Lambda=%.3f_Iter=%d.file"
		,ps_datos->i_N,ps_datos->d_lambda,i_iteracion);
	...
	*/
	...
	double (*pf_Din_Int)(ps_Red var, ps_Param par) = &Din_interes;
