...
for Carpeta in ["Transcritico/Norm_Maxima"]:
   ...
   # CÓDIGO PARA LEVANTAR ARCHIVOS DE UNA CARPETA CON TODOS LOS ARCHIVOS MEZCLADOS
    ...
    for nombre in Archivos_Datos[1::]:
        ...
        N = int(nombre.split("_")[2].split("=")[1])
        Cdelta = float(nombre.split("_")[3].split("=")[1])
        mu = float(nombre.split("_")[4].split("=")[1])
        if tipo not in SuperDiccionario[Carpeta].keys():
            SuperDiccionario[Carpeta][tipo] = dict()
            SuperDiccionario[Carpeta][tipo][N] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta][alfa] = [nombre]
        elif N not in SuperDiccionario[Carpeta][tipo].keys():
            SuperDiccionario[Carpeta][tipo][N] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta][alfa] = [nombre]
        elif mu not in SuperDiccionario[Carpeta][tipo][N].keys():
            SuperDiccionario[Carpeta][tipo][N][mu] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta][alfa] = [nombre]
        elif Cdelta not in SuperDiccionario[Carpeta][tipo][N][mu].keys():
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta] = dict()
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta][alfa] = [nombre]
        elif alfa not in SuperDiccionario[Carpeta][tipo][N][mu][Cdelta].keys():
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta][alfa] = [nombre]
        else:
            SuperDiccionario[Carpeta][tipo][N][mu][Cdelta][alfa].append(nombre)
    ...
    for AGENTES in [500]:
        ...
        Conjunto_Mus = list(SuperDiccionario[Carpeta][TIPO][AGENTES].keys())
        Conjunto_Mus.sort()
        ...
        # Dejo esta parte para después, cuando quiera volver a armar mapas de colores volveré
        # a revisar esto.
        ...
        for i_mu,MU in enumerate(Conjunto_Mus):
            ...
            # Para poder construir los siguientes arrays, necesito tener definido las listas de 
            # Cdelta y Alfas cosa de definir el tamaño de esas listas de antemano. Igual me parece
            # que la próxima vez que arme estas listas, lo mejor será buscar la forma de ir
            # appendeándola, para que tenga más sentido y no esté construido sobre la suposición de que
            # todas las combinaciones de mu, alfa y cdelta son del mismo tamaño.
            Conjunto_Cdelta = list(SuperDiccionario[Carpeta][TIPO][AGENTES][Conjunto_Mus[0]].keys())
            Conjunto_Alfa = list(SuperDiccionario[Carpeta][TIPO][AGENTES][Conjunto_Mus[0]][Conjunto_Cdelta[0]].keys())
            ...
            GradosTotales = np.zeros((len(Conjunto_Cdelta),len(Conjunto_Alfa)*len(SuperDiccionario[Carpeta][TIPO][AGENTES][Conjunto_Mus[0]][Conjunto_Cdelta[0]][Conjunto_Alfa[0]])))
            TiemposTotales = np.zeros((len(Conjunto_Cdelta),len(Conjunto_Alfa)*len(SuperDiccionario[Carpeta][TIPO][AGENTES][Conjunto_Mus[0]][Conjunto_Cdelta[0]][Conjunto_Alfa[0]])))
            ActividadesTotales = np.zeros((len(Conjunto_Cdelta),len(Conjunto_Alfa)*len(SuperDiccionario[Carpeta][TIPO][AGENTES][Conjunto_Mus[0]][Conjunto_Cdelta[0]][Conjunto_Alfa[0]])))
            ...
            for i_cdelta,CDELTA in enumerate(Conjunto_Cdelta):
                ...
                for i_alfa,ALFA in enumerate(Conjunto_Alfa):
                    ...
                    TideSi = np.zeros(len(SuperDiccionario[Carpeta][TIPO][AGENTES][MU][CDELTA][ALFA]))
                    Promedios = np.zeros(len(SuperDiccionario[Carpeta][TIPO][AGENTES][MU][CDELTA][ALFA]))
    ...
    #                    Opi0 = np.array([float(x) for x in Datos[1][1::]])
                        ...
                        TiemposTotales[i_cdelta,i_alfa*len(SuperDiccionario[Carpeta][TIPO][AGENTES][MU][CDELTA][ALFA])+numero] = Var.shape[0] # No debería ir multiplicado por 0,01?
                        ...
                        #----------------------------------------------------------------------------------------------------------------------------------
                            ...
                            # Esto me registra la simulación que va a graficar. Podría camiar
                            # los nombres y colocar la palabra sim en vez de iter.
                    ...
                    print("Tengo las opiniones finales mu={}, Cdelta={}".format(MU,CDELTA))
                    Tiempo()
