...
Este código lo terminé de armar el 23/12/2022. La idea era armar dos funciones, una que me sirva
para catalogar a los agentes según su distancia al primer agente, y la segunda para armar una lista
de agentes de los cuales voy a registrar su interés mientras evoluciona el sistema. Todo esto es
importante para estudiar la Ola de interés, es decir, la etapa del proyecto en la cual tomo a un
primer agente, le pongo un interés alto en el tópico, mientras el resto de los agentes tienen interés
nulo. Todo esto considerando un único tópico. La idea es ver cuánto tardan en llegar al pico de interés
los agentes según su distancia al primer agente.

La función que cataloga a los agentes según su distancia es Distancia_agentes.
La que arma la lista de agentes de los cuáles registrar datos es Lista_testigos.

Este código toma una matriz de adyacencia y luego escribe un archivo en la carpeta de Ola_interes.
La idea es poder usar los datos obtenidos y compararlos con una función que hace algo similar en
Python.
...
Este código lo terminé de armar el 21/09/2023. Acá hice dos cosas. Por un lado, puse
a prueba la función que calcula la distancia de un vector en un espacio no ortogonal.
Usando la misma definición que da Baumann al respecto, armé usando al definición de producto
escalar que coloca en el medio la matriz de Superposicion PHI. La función hace los cálculos
bien, hice las cuentas aparte y ningún problema. La función acá escrita tiene también unos
visualizar en el medio para controlar lo que va haciendo, pero no son parte de su funcionamiento.

También aproveché a probar cómo es el tema de poner varias condiciones distintas en un if.
Eso siempre me genera dudas, sé que lo voy a volver a probar en el futuro.
...
Este código lo armé el 30/11. Lo que construí es una función que calcula la varianza de un array de números.
Además sirve para calcular la varianza normalizada del vector o la varianza del vector como viene. Sólo devuelve
el valor de la varianza. La varianza la calcula dividiendo por N y no por N-1. Me parece que corresponde el tomar N
porque justamente yo tengo los datos de la población entera, mis N agentes. No es que tengo los datos de un subconjunto
de mis agentes.
