            ...
            for nombre in archivos:
                ...
                if repeticion < 3:
                    ...
                    for topico in range(T):
                        ...
                        X = np.arange(Testigos.shape[0])*0.5
                        for sujeto in range(int(Testigos.shape[1])):
                            plt.plot(X,Testigos[:,sujeto*T+topico],color = "tab:purple", linewidth = 2, alpha = 0.4)
                        plt.xlabel("Tiempo")
                        plt.ylabel(r"$x^i$")
...
def Punto_fijo_vs_Kappa(DF,path,carpeta,T,
                        SIM_param_x, SIM_param_y, ID_param_y, ID_param_extra_1):
    
    AGENTES = int(np.unique(DF["n"]))
    EXTRAS = float(np.unique(DF["Extra"]))
    
    Arr_param_x = np.array([1.5,2,2.5,3])
    Arr_param_y = np.unique(DF["parametro_y"])
    
    Tupla_total = [(param_x,j,param_y) for param_x in Arr_param_x
    ...
    plt.xlabel(r"${}$".format(SIM_param_y))
    plt.ylabel(r"$<x>_f$")
    plt.grid(alpha = 0.5)
    
    
    for PARAM_X,j,PARAM_Y in Tupla_total:
        ...
        X_i = np.ones(archivos.shape[0]) * PARAM_Y
        ...
        for i,nombre in enumerate(archivos):
            ...
            Y_i[i] = np.mean(np.array(Datos[5][:-1],dtype="float"))/PARAM_Y # Tomo los intereses finales y les tomo un promedio
        ...
        if j == Arr_param_x.shape[0]-1:
#            X = X + rng.normal(scale = 0.2, size = X.shape)
            plt.scatter(X,Y, label = r"${} = {}$".format(SIM_param_x,PARAM_X), s = 200)
    ...
    direccion_guardado = Path("../../../Imagenes/{}/Puntofijovs{}_N={:.0f}_{}={:.1f}.png"
                              .format(carpeta,ID_param_y,AGENTES,ID_param_extra_1,EXTRAS))
    plt.legend()
    plt.savefig(direccion_guardado ,bbox_inches = "tight")
    plt.close("Puntofijo")
