...
int main(int argc, char *argv[]){
	...
	param->testigos = param->N; // Esta es la cantidad de agentes de cada distancia que voy registrar
	...
	red->Ady = (int*) calloc( 2+param->N*param->N, sizeof(int) ); // Matriz de adyacencia de la red. Determina quienes están conectados con quienes
	...
	char TextOpi[355];
	sprintf(TextOpi,"../Programas Python/Evolucion_temporal/1D/Opiniones_N=%d_kappa=%.1f_beta=%.2f_cosd=%.2f_Iter=%d.file"
	...
	sprintf(TextTestigos,"../Programas Python/Evolucion_temporal/1D/Testigos_N=%d_kappa=%.1f_beta=%.2f_cosd=%.2f_Iter=%d.file"
		,param->N, param->kappa, param->beta, param->Cosd, iteracion);
	...
	for(int i=0; i<ancho_ventana-1; i++){
		RK4(red->Opi, func_dinamica, red, param); // Itero los intereses
	...
	while(contador < param->Iteraciones_extras && pasos_simulados < pasos_maximos){
		...
		do{
			...
			if( pasos_simulados%100==0 ){
				...
				for(int j=0; j<param->N*param->T; j++) red->Dif[i_j+2] = red->Prom_Opi[ j+param->N*param->T+2 ] - red->Prom_Opi[j+2];
				...
				for(int j=0; j<param->N*param->T; j++) red->Prom_Opi[i_j+2] = red->Prom_Opi[ j+param->N*param->T+2 ];
